...
def visualize_pointclod_trajectory(path, data_tensor, all_angles=False, elev=30, azim=225) -> None:
    # Create a figure and axis
    fig, ax = plt.subplots()
    ax = fig.add_subplot(111, projection='3d')  # Specify 3D projection
    # Initialize an empty scatter plot (assuming 3D point cloud)
    scatter = ax.scatter([], [], [], c='b', marker='o', s=5)
    ...
    def update(frame):
        ...
        # Change the viewing angle for the last 10 frames
        if frame >= len(data_tensor):
            angle_step = 36  # Degrees to rotate in each step
            ax.view_init(elev=(elev + (frame - len(data_tensor)) * angle_step) % 360, azim=azim)
            return

        ax.cla()  # Clear the previous frame
        scatter = ax.scatter(data_tensor[frame, :, 0], data_tensor[frame, :, 1], data_tensor[frame, :, 2], c='b', marker='o', s=5)
        ax.set_title(f'Frame {frame + 1}/{len(data_tensor)}')
        ax.axis('off')
    
    # Create the animation
    total_frames = len(data_tensor) + 10
    # Create the animation
    animation = FuncAnimation(fig, update, frames=total_frames, interval=200, repeat=True)
