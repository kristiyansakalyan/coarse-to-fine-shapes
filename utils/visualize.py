...
def visualize_pointcloud(
    points, normals=None, out_file=None, show=False, elev=30, azim=225
):
...
def visualize_pointclod_trajectory(
    path, data_tensor, all_angles=False, elev=30, azim=225
) -> None:
    ...
    def update(frame):
        ...
        if frame >= len(data_tensor):
            ...
            ax.view_init(
                elev=(elev + (frame - len(data_tensor)) * angle_step) % 360, azim=azim
            )
            return
    ...
    animation = FuncAnimation(
        fig, update, frames=total_frames, interval=200, repeat=True
    )
