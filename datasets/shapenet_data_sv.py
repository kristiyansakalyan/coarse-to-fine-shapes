...
def _convert_categories(categories):
    ...
    synsets = [
        label_to_synset[c] if c in label_to_synset.keys() else c for c in categories
    ]
    return synsets
    ...
    def __init__(
        ...
        for i in range(len(self.synsets)):
            ...
            for mid in tqdm(self.all_mids):
                ...
                cams_pths = list(
                    (self.root / syn / mid.split("/")[-1]).glob("*_cam_params.npz")
                )
                if len(cams_pths) < 20:
                    continue
                ...
                sv_points_group = []
                img_path_group = []
                (self.cache_dir / (mid.split("/")[-1])).mkdir(
                ...
                for i, cp in enumerate(cams_pths):
                    ...
                    cache_pth = str(
                        self.cache_dir
                        / mid.split("/")[-1]
                        / os.path.basename(depth_minmax_pth)
                    )
        ...
        if (
            ...
            self.all_points_mean = (
                self.all_points.reshape(-1, 3).mean(axis=0).reshape(1, 1, 3)
            )
            self.all_points_std = (
        ...
        self.all_points_sv = (
            self.all_points_sv - self.all_points_mean
        ) / self.all_points_std

    def get_pc_stats(self, idx):
        return self.all_points_mean.reshape(1, 1, -1), self.all_points_std.reshape(
            1, 1, -1
        )
    ...
    def __getitem__(self, index):
        ...
        data = torch.cat(
            [
                ...
                torch.zeros(
                    sv_points.shape[0], self.npoints - idxs.shape[0], sv_points.shape[2]
                ),
            ],
            dim=1,
        )
        ...
        if self.split != "train" and self.get_image:
            ...
            for n in range(self.all_points_sv.shape[1]):
                img = (
                    torch.from_numpy(plt.imread(self.imgs[index][n]))
                    .float()
                    .permute(2, 0, 1)[:3]
                )
...
class DepthToSingleViewPoints(object):
    ...
    def __call__(self, depth_pth, depth_minmax_pth):
        ...
        depth_img = (
            depth_img * (np.max(depth_minmax) - np.min(depth_minmax))
            + np.min(depth_minmax)
        ) * mask
        ...
        pcd = o3d.geometry.PointCloud.create_from_depth_image(
            depth_im, intr, self.cam_ext, depth_scale=1.0
        )
