...
def train(gpu, opt, output_dir, noises_init):
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        if (epoch + 1) % opt.vizIter == 0 and should_diag:
            ...
            model.eval()
            with torch.no_grad():
                logger.info(f"New x chain : {x.shape}")
                x_gen_eval = model.gen_samples(new_x_chain(x, 2).shape, x.device, clip_denoised=False)
                logger.info("After gen_eval")
                x_gen_list = model.gen_sample_traj(new_x_chain(x, 1).shape, x.device, freq=40, clip_denoised=False)
                logger.info("After x_gen_list")
            ...
            logger.info("Before 3rd visualization")
...
def parse_args():
    ...
    parser.add_argument('--vizIter', default=1, help='unit: epoch')
